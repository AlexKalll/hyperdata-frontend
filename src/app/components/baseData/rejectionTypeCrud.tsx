...
export function RejectionTypeCRUD() {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [currentItem, setCurrentItem] = useState<any>(null);
  const queryClient = useQueryClient();
  const dataService = useDataService("rejectionType");
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["rejection-type"] });
  ...
  const columns: ColumnDef<RejectionType>[] = [
    ...
    { accessorKey: "description", header: "Description" },
    {
      accessorKey: "",
      header: "Action",
      cell: function RejectionTypeActionCell({ row }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <p className="mb-8 font-bold ">Add New Rejection Type</p>
          </DialogHeader>
          <AddBasedataForm
            isOpen={isDialogOpen}
