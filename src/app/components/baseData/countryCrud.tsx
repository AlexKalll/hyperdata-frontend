...
export function CountryCRUD() {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [currentItem, setCurrentItem] = useState<any>(null);
  const queryClient = useQueryClient();
  const dataService = useDataService("country");
  ...
  const { data: countryData, isLoading: iscountryLoading } = useBasedataAll({
    page,
    pageSize,
    servicename: "country",
  ...
  const paginatedcountryData = countryData?.data.result || [];
  const countryTotalElements = countryData?.data?.total || 0;
  const countryTotalPages = countryData?.data.totalPages || 0;
  const companyStartRecord = paginatedcountryData.length
    ? (page - 1) * pageSize + 1
    : 0;
  const companyEndRecord = Math.min(page * pageSize, countryTotalElements);
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["countrys"] }),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <p className="mb-8 font-bold ">Add New Country</p>
          </DialogHeader>
          <AddBasedataForm
            isOpen={isDialogOpen}
      ...
      <DataTable
        ...
        pagination={{
          pageCount: countryTotalPages,
          page,
          setPage: handlePageChange,
          pageSize,
          setPageSize: handlePageSizeChange,
          showingText:
            countryTotalElements > 0
              ? `Showing ${companyStartRecord} to ${companyEndRecord} out of ${countryTotalElements} records`
              : "",
        }}
      />
    </div>
